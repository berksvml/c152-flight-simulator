    ...
    const FString& Parameters)
{
    ...
    TestTrue(
        ...
        IsAtmosphereValueNear(Atmosphere.TemperatureKelvin, 288.15, 1.0e-9));
    TestTrue(
        TEXT("Sea-level pressure is 101325 Pa"),
        IsAtmosphereValueNear(Atmosphere.PressurePascals, 101325.0, 1.0e-6));
    TestTrue(
        TEXT("Sea-level density is approximately 1.225 kg/m^3"),
        IsAtmosphereValueNear(Atmosphere.DensityKilogramsPerCubicMeter, 1.225, 1.0e-4));
    TestTrue(
        TEXT("Sea-level speed of sound is approximately 340.3 m/s"),
        IsAtmosphereValueNear(Atmosphere.SpeedOfSoundMetersPerSecond, 340.3, 0.1));
    ...
    const FString& Parameters)
{
    ...
    TestTrue(
        ...
        IsAtmosphereValueNear(AtFiveKilometers.TemperatureKelvin, 255.65, 1.0e-9));
    TestTrue(
        TEXT("Pressure at 5 km agrees with the standard atmosphere"),
        IsAtmosphereValueNear(AtFiveKilometers.PressurePascals, 54019.9, 1.0));
    TestTrue(
        TEXT("Pressure at 11 km agrees with the standard atmosphere"),
        IsAtmosphereValueNear(AtElevenKilometers.PressurePascals, 22632.1, 1.0));
    TestTrue(
        TEXT("Density at 11 km is approximately 0.364 kg/m^3"),
        IsAtmosphereValueNear(
